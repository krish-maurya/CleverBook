...
const MONGO_URI =
  process.env.MONGO_URI || "mongodb://localhost:27017/cleverbooks";
...
notificationQueue.process("send-notification", async (job) => {
  ...
  console.log(
    `[NotificationWorker] Processing job ${job.id} for AWB ${awbNumber} (attempt ${job.attemptsMade + 1})`,
  );
  ...
  try {
    ...
    console.log(
      `[NotificationWorker] Successfully sent notification for AWB ${awbNumber}`,
    );
    return result;
  } catch (err) {
    ...
    console.error(
      `[NotificationWorker] Failed to send notification for AWB ${awbNumber}:`,
      err.message,
    );
    ...
    if (job.attemptsMade >= 2) {
      ...
      console.log(
        `[NotificationWorker] Moving AWB ${awbNumber} to dead letter queue`,
      );
    } else {
      updateData.status = isRateLimit ? "RATE_LIMITED" : "RETRIED";
    }

    await Notification.findByIdAndUpdate(notificationId, updateData);

    // 🔥 IMPORTANT: add delay for 429
    if (isRateLimit) {
      const delay = 5000 * Math.pow(2, job.attemptsMade); // exponential delay
      console.log(
        `[NotificationWorker] Delaying retry by ${delay}ms due to rate limit`,
      );
...
notificationQueue.on("failed", async (job, err) => {
  if (job.attemptsMade >= 3) {
    console.log(
      `[NotificationWorker] Job ${job.id} permanently failed after ${job.attemptsMade} attempts`,
    );
...
deadLetterQueue.process("dead-letter", async (job) => {
  console.log(
    `[NotificationWorker] Dead letter received: AWB ${job.data.data.awbNumber}`,
  );
...
async function start() {
  ...
  console.log(
    `[NotificationWorker] Webhook URL: ${process.env.WEBHOOK_URL || "https://webhook.site/d8c272b7-4b88-4aa4-9b05-fc23c0d836d9"}`,
  );
